...
function buildFirebaseConfig() {
  return { ...rawFirebaseConfig } as any;
}
...
export async function handleGoogleRedirectResult(): Promise<User | null> {
  ...
  try {
    ...
    if (wasRedirectPending && !auth.currentUser) {
      ...
      document.cookie = 'redirect_login_pending=; path=/; max-age=0; SameSite=Lax';
      throw new Error("Login session lost during redirect. Your browser's tracking prevention blocked the login. Please allow popups for this site and try signing in again, or use Chrome/Firefox.");
    }
