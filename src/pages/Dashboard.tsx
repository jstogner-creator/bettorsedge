...
export function Dashboard({
  ...
  */
  ...
  // Auto-generate missing predictions locally when games are loaded but predictions are missing
  const lastAutoGeneratedRef = useRef<string>("");
  useEffect(() => {
    if (!games || games.length === 0) return;
    
    const dateStr = format(selectedDate, "yyyy-MM-dd");
    const missingGames = games.filter(g => !savedPredictions[g.id]);
    
    if (missingGames.length === 0) return;
    
    const generatedKey = `${dateStr}-${games.map(g => g.id).join(",")}`;
    if (lastAutoGeneratedRef.current === generatedKey) return;
    lastAutoGeneratedRef.current = generatedKey;
    
    const generate = async () => {
      console.log(`[Dashboard] Auto-generating predictions locally for ${missingGames.length} games on ${dateStr}`);
      const newPredictions: Record<string, Prediction> = {};
      
      for (const game of missingGames) {
        try {
          const pred = await bettorsEdge.analyzeMatchup(game, dateStr, null, []);
          if (pred) {
            newPredictions[game.id] = pred;
          }
        } catch (err) {
          console.warn(`[Dashboard] Failed to auto-generate local prediction for game ${game.id}:`, err);
        }
      }
      
      if (Object.keys(newPredictions).length > 0) {
        setSavedPredictions(prev => ({ ...prev, ...newPredictions }));
        setAllPredictions(prev => ({ ...prev, ...newPredictions }));
      }
    };
    
    const timer = setTimeout(generate, 1500);
    return () => clearTimeout(timer);
  }, [games, selectedDate, savedPredictions]);
  ...
  const handleReanalyzeSingleGame = async (game: Game) => {
    ...
    try {
      ...
      if (prediction && game.id && !cancelAnalysisRef.current[targetLeague]) {
        // Save immediately to Firestore, catching errors if offline or database is not reachable
        try {
          await bettorsEdge.savePrediction(game.id, prediction);
        } catch (saveErr) {
          console.warn(`[Dashboard] Failed to save prediction to Firestore for game ${game.id}:`, saveErr);
        }
